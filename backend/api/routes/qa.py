...
# ------------------------------
# Utilities for hybrid retrieval
# ------------------------------
...
def _select_offer_chunks_from_db(
    ...
    insurer_only: Optional[str]
) -> List[Dict[str, Any]]:
    """
    Returns a list of rows: {file_id, filename, chunk_index, text, insurer_code}
    from Postgres offer_chunks for this share/batch (OFFERS are the source of truth).
    """
...
def _detect_latvian(text: str) -> bool:
    ...
    return any(c in latvian_chars for c in text)
...
# --------------------------------------------------------------------
# Scoped Q&A (/ask-share): OFFERS from DB + optional T&C citations
# --------------------------------------------------------------------
...
def ask_share_qa(req: QAAskRequest, conn = Depends(get_db)):
    ...
    try:
        ...
        # ---------- Optional: harvest a few T&C citations from vector store ----------
        extra_labels: List[str] = []
        try:
            tc_vs_id = get_tc_vs(conn, org_id, "insurer_tc")
            assistant_id = os.getenv("ASSISTANT_ID_QA")
            if tc_vs_id and assistant_id:
                vector_store_ids = [tc_vs_id]
                ...
                run = client.beta.threads.runs.create(
                    ...
                    instructions="Search insurer T&C for brief supporting references only.",
                )
                        ...
                        for c in m.content:
                            ...
                            for ann in (c.text.annotations or []):
                                ...
                                if t == "file_citation" and getattr(ann, "file_citation", None):
                                    ...
                                    fname = (
                                        getattr(fp, "display_name", None)
                                        or getattr(fp, "filename", None)
                                        or getattr(fp, "title", None)
                                    )
                                if fid or fname:
                                    _append_label(fid, fname)

                    extra_labels = source_labels[:4]
        except Exception as e:
            print(f"[qa] optional VS citation harvest warn: {e}")

        # Merge DB sources + optional T&C labels (unique, DB first)
...
# --------------------------------------------------------------------
# Visibility audit for a share (unchanged)
# --------------------------------------------------------------------
...
def _reembed_file(file_id: int, conn) -> dict:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            retrieval_file_id = meta.get("retriaeval_file_id") or meta.get("retrieval_file_id")
